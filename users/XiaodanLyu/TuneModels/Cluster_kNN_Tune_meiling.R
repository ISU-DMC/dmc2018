...
setwd(WD)
#T1 <- Sys.time()
Data <- read_rds("Cache/alltrain_sub_prc_may15.rds.rds")
...
Tune <- train(y=ifelse(Train$units>0, 1, 0),
...
write_rds(Pred, sprintf("%sPredJankNN_Month%s_C%s_%s.rds", Result_Dir_True, Month, Cluster, k))
...
# write_rds(Error, sprintf("%sErrorkNN_Month%s_C5_%s.rds",Result_Dir_True, Month,  k ))
