...
demo_info = layer_concatenate(c(encode_demo_cate, demo_input_numeric))  %>%
  layer_dense(units = 16, name = 'demo_info') 

history_input = layer_input(shape = list(maxlen,1), name = 'feed_history')
seq_info = layer_concatenate(c(history_input, layer_repeat_vector(demo_info, maxlen)), axis = 2) %>%
  #  layer_embedding(input_dim = max(mat.x[,2:93], na.rm = T), 
  #                  output_dim = 4, input_length = maxlen,
  #                  name = 'encode_history') %>%
  layer_lstm(64, return_sequences =T) %>%
  layer_lstm(32, name = 'sequential_info')

next_sales_output = seq_info %>%
  layer_dense(next_len, name = 'forecast_next_sale', activation = 'softplus')

predSOT_output = layer_concatenate(c(demo_info, seq_info)) %>%
#predSOT_output = seq_info %>%
  layer_dense(16, name = 'merge_info') %>%
  layer_dense(8, name = 'hidden_dense') %>%
...
st.time = proc.time()
mat_predSOT = rep(NA, nrow(dat.sale.test))
for(whichPidxSize in lst_whichPidxSize){
  ...
  new.demo.numer = c(dat.sale.test$stock[whichPidxSize],             # stock
    ...
    mat_predSOT[whichPidxSize] = predict(mdl, list(feed_demo_categorical = new.demo.cate, 
...
set.seed(0)
rndGuss = sample.int(31, length(predSOT), replace = T)
(rndGuss - dat.sale.test$sOT) %>% abs %>% sum(na.rm = T) %>% sqrt
(rndGuss- dat.sale.test$sOT) %>% abs %>% mean(na.rm = T)

(predSOT - dat.sale.test$sOT) %>% hist(breaks = 50, main = 'pred - true sold out', xlim = c(-31,31))
(rndGuss - dat.sale.test$sOT)%>% hist(breaks = 50, main = 'rndGuss - true sold out', xlim = c(-31,31))
