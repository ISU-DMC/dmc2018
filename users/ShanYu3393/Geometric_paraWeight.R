...
Fold_train <- function(i,Train_10_12,ord,set) {
  ...
  W_train <- train %>% group_by(pid,size,date) %>% tally()
  W_test <- test %>% group_by(pid,size,date) %>% tally()
  
  train <- train  %>% left_join(W_train)
  test <- test %>% left_join(W_test)

  W_train <- train$n
  W_test <- test$n
  
  train <- train  %>% select(-pid,-size,-date,-n)
  test <- test %>% select(-pid,-size,-date,-n)
  ...
  for(j in 1:num_paras) {
    ...
    fit_tmp <- xgboost(data=data.matrix(train[,-1]), label=train[,1], 
                 objective="binary:logistic", weight = 1/W_train,
                 nrounds = paras[j,1], max_depth = paras[j,2], 
                 subsample = paras[j,3])
    
    pred_test <- predict(fit_tmp, newdata = data.matrix(test[,-1]))
    MSPE1[j] <- sum(1/W_test*(pred_test - test[,1])^2)
    MSPE2[j] <- pr.curve(test[,1],pred_test,1/W_test,1/W_test)$auc.integral
...
TrainGeo = readRDS('/Users/shanyu/Desktop/DMC/Geo_LLR_alltrain_subfeatures_may14.rds') %>%
...
write.csv(Result,paste0('Result/GeoLowCVErrWeight_3_',cluster,'.csv'))    
