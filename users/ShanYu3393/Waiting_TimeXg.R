...
## --------------------- split 2018-01
# train
Train=Total_W[Total_W$startdate <= ymd('2017-12-31'),] %>% select(-startdate)
...
# test
Test=Total_W[Total_W$startdate > ymd('2017-12-31'),]
Test$startdate=NULL
Test$size=NULL
Test$startdate=NULL
Test=data.matrix(Test)

fitted = xgboost(data=Train, label=Train[,1], max_depth=40, eval_metric="mae",
                 weight=Weight_train,
        subsample=0.6, eta=0.3, objective="count:poisson", nrounds = 160)

Pred=predict(fitted, Test[,-1])
Pmae=mean(abs(Test[,1]-Pred))
