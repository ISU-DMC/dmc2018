...
TrainGeo = readRDS('/Users/shanyu/Desktop/DMC/Geo_LLR_alltrain_subfeatures_may14.rds') %>%
  mutate_if(is.character,as.factor) %>% filter( date.day <= 95) %>%
...
Best <- read.csv('BestParameter.csv')
...
Test_feature <- Test_data %>% filter(Cluster_9==cluster, date.day > 95) %>%
  dplyr::select(-(Cluster_2:Cluster_9)) %>% select(-pid, -size, -date)
TRUE_data <- Test_data  %>% filter(Cluster_9==cluster, date.day > 95) %>%
  dplyr::select(-(Cluster_2:Cluster_9)) %>% select(pid, size, date, units)
